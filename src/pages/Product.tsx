...
export function Product() {
  ...
  const { addItem, wishlist, toggleWishlist, products, cart, setBagOpen } = useStore();
  ...
  function add() {
    if (!size) { setFormError('Choose an available size before adding to your bag.'); return false; }
    if (lineLimitReached || inBag + quantity > CART_MAX_QUANTITY) { setFormError(CART_LIMIT_MESSAGE); return false; }
    if (quantity > remaining) { setFormError('That quantity is already in your bag or no longer available.'); return false; }
    const added = addItem(product!, size, quantity);
    setFormError(added ? '' : 'Could not add this size. Review your bag and availability.');
    return added;
  }
  function buyNow() { if (add()) { setBagOpen(false); navigate('/checkout'); } }
