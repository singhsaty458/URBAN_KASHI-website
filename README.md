...
- Three-slide winter coat-fashion **video hero** with horizontal sliding transitions, touch swipes, muted inline playback, matching poster fallbacks and a persistent **Pause motion** preference; a gently moving two-photo lookbook. Autoplay/video pause on hover, keyboard focus, hidden tabs and offscreen content. Reduced-motion users get static posters; supported data-saver connections do not load video. Licensed stock/demo footage is not actual inventory. Sources and replacement instructions: [docs/HERO_VIDEOS.md](docs/HERO_VIDEOS.md).
...
**Winter-video update verification (8 September 2026):** production build and all **66 desktop/mobile Chromium browser tests passed**, including real local H.264 playback, muted/inline behavior, inactive/offscreen pause, reduced-motion/data-saver posters, decode/autoplay fallback and horizontal-swipe transitions. The running website was separately checked for real playback at desktop/mobile viewport sizes. No full API suite or live payment-provider verification was performed for this frontend update.

Historical backend validation: **154 API tests passed**, including upload and lifecycle coverage; this is not a new API-suite run. The earlier 52-test browser run had been unconfirmed; the current 66-test browser result is recorded above. Payment service tests use injected fakes; production gateway adapter and browser payment-contract tests use intercepted/mock responses, not external provider verification. No live/sandbox payments, external webhook delivery or real refunds have been tested. The [Dockerfile](Dockerfile) is supplied; a container build has not been verified.
