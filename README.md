...
- Consistent **black / champagne gold / white** theme across home, shop, product, wishlist, account, checkout, order, editorial and admin pages. Dark header/footer and dialog headers frame readable white/light-neutral content, gold actions and accessible dark-gold accents. Semantic error/payment-status colours remain distinct. Shared theme: [src/storefront-theme.css](src/storefront-theme.css).
- Original modern fashion homepage with bold typography, editorial photography and responsive category cards. Inspired by contemporary clothing storefront patterns, not a copied Shopify theme or a Shopify integration.
- Three-slide **leather/bomber jacket video hero** with horizontal transitions, touch swipes, muted inline playback, matching posters and a persistent **Pause motion** preference. Hover does not pause footage or autoplay. Keyboard focus holds carousel position without freezing video; user pause, hidden tabs and offscreen content stop playback. Reduced-motion users get static posters; supported data-saver connections do not load video. No parka/baseball clip is claimed. Licensed stock/demo footage is not actual inventory. Sources and replacement instructions: [docs/HERO_VIDEOS.md](docs/HERO_VIDEOS.md).
- Real catalogue-powered **Your next rotation** tabs, four-piece selections, product/wishlist links and loading/error/empty states. Homepage composition remains scoped; shared shopping controls and theme apply across routes.
...
- Product cards show larger wrapping names/prices, an explicit size selector, **Add to bag** and **Buy now** below the photograph. No size is selected automatically; sold-out sizes and cart/stock limits are respected. Cards add one unit; product details offer quantity selection and both actions. Buy now adds the selection and opens normal authenticated checkout **with existing bag items preserved**, not an isolated or automatic paid order. Prices/stock are still revalidated by the server.
...
**Shared-theme / jacket / purchase-control verification (8 September 2026):** production build and all **80 desktop/mobile Chromium browser tests passed**. Coverage includes cross-page colours and dialogs; 320/390/768/1440px long-name/price/action alignment; explicit/custom size selection and keyboard focus restoration; stock, ten-unit and twenty-line limits; card/detail Buy now with preserved bag and authentication gate; and actual H.264 frame progress on hover/focus. All three replacement jacket clips decode. Explicit pause, offscreen/reduced-motion/data-saver behavior, error posters and swipe transitions remain covered. Anonymous read-only desktop/mobile previews of the running website were captured separately. Tests use the isolated temporary database on port 4180, not real store records. No full API suite or live payment-provider verification was performed for this frontend update.

Historical backend validation: **154 API tests passed**, including upload and lifecycle coverage; this is not a new API-suite run. The earlier 52-test browser run had been unconfirmed; the current 80-test browser result is recorded above. Payment service tests use injected fakes; production gateway adapter and browser payment-contract tests use intercepted/mock responses, not external provider verification. No live/sandbox payments, external webhook delivery or real refunds have been tested. The [Dockerfile](Dockerfile) is supplied; a container build has not been verified.
